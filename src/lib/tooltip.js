 ...
 * @param {{ content?: string, [key: string]: any }} [params]
 */
export default function tooltip(node, params = {}) {
  ...
  const tip = tippy(node, { content: content ?? "", ...params });

  return {
    // If the props change, let's update the Tippy instance:
    /**
     * @param {{ content?: string, [key: string]: any }} newParams
     */
    update: (newParams) => tip.setProps({ content: content ?? "", ...newParams }),
